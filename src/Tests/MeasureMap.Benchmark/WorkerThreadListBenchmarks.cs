...
namespace MeasureMap.Benchmark
{
    public class WorkerThreadListBenchmarks
    {
        public void RunBenchmarks()
        {
            ...
            benchmark.Task("StartNew Thread", ctx => threads.StartNew(ctx.Get<int>(ContextKeys.Iteration), _ => new Result(), WorkerThread.Factory));
            benchmark.Task("StartNew Task", ctx => threads.StartNew(ctx.Get<int>(ContextKeys.Iteration), _ => new Result(), WorkerTask.Factory));
